...
class ALIMDiffSpider(scrapy.Spider):

    name = CONST.ALI_YUN_M_P_SPIDER_NAME  # 定义爬虫名

    allowed_domains = [CONST.ALI_YUN_DOMAIN]  # 接受处理的域名

    crawl_type = CrawlType.puppeeter

    def start_requests(self):
        args_dict = {'title': 'Pupeeter_M页面'}
        yield PuppeeterRequest(url=CONST.WU_BA_M_URL, callback=self.parse, screen_shot=True, wait_time=20,
                              r_dict=args_dict)
